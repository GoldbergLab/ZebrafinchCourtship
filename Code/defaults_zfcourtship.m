function handles = defaults_zfcourtship(handles)
% Default settings

% GENERAL SETTINGS
handles.TooLong = 400000000; % Number of points for a file to be considered too long for loading automatically
handles.FileString =        {'*chan%d??.wav', '*chan%d??.wav', '*chan%d??.nc', '*chan%d??.nc'}; % File search string; must include a string formatting expression to handle an integer channel #, such as %02d for integers zero-padded to 2 digits, or %d for unpadded integers.handles.DefaultFileLoader = 'Intan_Bin'; % Default file loader. Choose from egl_* files.
handles.DefaultFileLoader = {'WaveRead',      'WaveRead',      'Intan_Nc',     'Intan_Nc'};     % Default file loader - either a single char array, or a cell array containing one per channel. Choose from egl_* files.
handles.DefaultChannelNumber = 3; % Default number of channels, not counting the sound channel
